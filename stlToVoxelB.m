% Edited by G. Kook
% Reference: https://stackoverflow.com/questions/51158078/load-stl-file-in-matlab-and-convert-to-a-3d-array

function model = stlToVoxelB(grid_x,grid_y,grid_z,dir_in)
    TR = stlread(dir_in);
    model = inpolyhedron(TR.ConnectivityList,TR.Points,grid_x,grid_y,grid_z);
    model = permute(model, [2 1 3]);


% Reference: https://kr.mathworks.com/matlabcentral/fileexchange/37856-inpolyhedron-are-points-inside-a-triangulated-volume
...
N = numel(find(~emptyMask));
iter1 = 0;
f = waitbar(0,'Bro falls!','Name','Converting stl file into voxel...','CreateCancelBtn','setappdata(gcbf,''canceling'',1)');
setappdata(f,'canceling',0);
s = clock;  
for i = find(~emptyMask)
    if getappdata(f,'canceling')
        delete(f)
        error('User cancelled importing stl.');
    end
    iter1 = iter1 + 1;
    if iter1 >= 2
        esttime = etime(clock,s)/(iter1-1)*(N-iter1+1);   
        waitbar(iter1/N,f,['Remaining time = ',num2str(esttime,'%4.0f'),' s']);
    end
